...
smd.loc[:, 'cast'] = smd['cast'].apply(lambda x: [str.lower(i.replace(" ", "")) for i in x])
smd.loc[:, 'director'] = smd['director'].astype('str').apply(lambda x: str.lower(x.replace(" ", "")))
...
smd.loc[:, 'keywords'] = smd['keywords'].apply(lambda x: [str.lower(i.replace(" ", "")) for i in x])
...
def get_movie_details(movie_title):
    ...
    if 'results' in data and data['results']:
        ...
        return {
            ...
            'poster_path': 'https://image.tmdb.org/t/p/w500/' + movie_details.get('poster_path', ''),
        }
...
def get_recommendations(movie_title):
    ...
    recommendations_url = 'https://api.themoviedb.org/3/movie/{movie_id}/recommendations'
    ...
    if 'results' in search_data and search_data['results']:
        ...
        recommendations_response = requests.get(recommendations_url.format(movie_id=movie_id), params=recommendations_params)
        recommendations_data = recommendations_response.json()

        recommendations = []
        for result in recommendations_data.get('results', [])[:10]:
